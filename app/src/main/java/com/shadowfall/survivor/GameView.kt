...
class GameView(context: Context) : View(context) {
    ...
    init { isFocusable = true; resetRun() }

    private fun resetRun() {
        playerX = 0f; playerY = 0f; hp = 100f; maxHp = 100f; xp = 0f; xpNeed = 30f
        level = 1; attack = 18f; moveSpeed = 260f; room = 1; roomKills = 0; totalKills = 0; roomsCleared = 0
        ...
        phase = Phase.COMBAT; enemies.clear(); orbs.clear(); spawnTimer = 0.5f; dashCooldown = 0f; dashInvulnerable = 0f
    }
    ...
    private fun update(dt: Float) {
        ...
        else if (phase == Phase.BOSS) { updatePlayer(dt); updateEnemies(dt); updateProjectiles(dt); if (enemies.isEmpty()) { victory = true; phase = Phase.VICTORY; saveRun(true) } }
    }
    ...
    private fun damagePlayer(amount:Float){if(dashInvulnerable>0f||gameOver)return;hp-=amount;if(hp<=0f){hp=0f;gameOver=true;saveRun(false)}}
    ...
    private fun handleDown(index:Int,id:Int,x:Float,y:Float){if(choosingUpgrade){if(y in 200f..510f){when{y<300f->maxHp+=25f;y<400f->attack+=8f;else->moveSpeed+=35f};hp=min(maxHp,hp+25f);choosingUpgrade=false};return};if(phase==Phase.CHEST){val cx=width/2f;val cy=height/2f;if(x in cx-110f..cx+110f&&y in cy-90f..cy+90f)continueFromChest();return};if(gameOver||victory){resetRun();return};if(x>width-145&&y<80){paused=!paused;return};if(x<210&&y>height-210){joystickId=id;updateJoystick(x,y);return};if(x>width-145&&y>height-195){attackDown=true;return};if(x>width-260&&y>height-100){dashDown=true;return}}
    ...
    private fun saveRun(won:Boolean){save.edit().putInt("bestLevel",max(save.getInt("bestLevel",1),level)).putInt("bestKills",max(save.getInt("bestKills",0),totalKills)).putInt("roomsCleared",max(save.getInt("roomsCleared",0),roomsCleared+if(won)3 else 0)).apply()}
}
